...
- [ ] アニメーションの `タイミング` に引用符 `「」` を付けていないか（付けると ratio=0.5 になる）（PITFALLS.md 7-4）
...
### 7-3. テキストマッチ方式はアニメーションタイミングがずれる

`_find_timing_ratio()` はタイミングテキストの**文字数位置 / 台本全文字数**で発火比率を計算する。
しかし TTS の音声は文字数と時間が比例しない（読点の間、長い単語、数式の読み上げ速度の違いなど）。
そのため**テキストマッチで得た比率 ≠ 実際に Leda がその言葉を発する時刻**になることがある。

**症状：** アニメーションが 1〜3秒早い・遅い。スライド末尾付近のセリフほどズレが大きい。

**根本対処：動画を確認して % を直接指定する**

1. 生成した動画を再生し、アニメーションをかけたいセリフの**タイムスタンプ**を確認する
2. そのスライドの開始タイムスタンプ（edit.md のタイムスタンプ欄）を引いて経過秒を求める
3. スライド尺（次スライド開始 − 自スライド開始）で割って割合を計算し `XX%` 形式で指定する

```
例：スライド④ 2:31〜3:32（尺61秒）、発火させたいのが 3:18
  経過 = 3:18 − 2:31 = 47秒
  比率 = 47 / 61 ≈ 77%
  → タイミング：77%
```

`%` 形式は `_find_timing_ratio()` が最初に解釈するため、テキスト検索より高速かつ確実。

---

### 7-4. タイミングに引用符「」を付けてはいけない

`_find_timing_ratio()` は `タイミング` テキストを `normalize_for_tts()` で正規化して
台本内の位置を検索する。**テキストを `「」` で囲むと、正規化後に `「` が残り
台本と一致しなくなる → デフォルト 0.5（50%）へフォールバックする。**

**NG（引用符付き）：**
```
- タイミング：「ステップ2。底 a を確認する」
```
→ `normalize_for_tts` 後 `「ステップに。底 エー を確認する」` となり
  台本には `「` がないのでマッチせず ratio=0.5 になる。

**OK（引用符なし）：**
```
- タイミング：ステップ2。底 a を確認する
```
→ 台本の対応箇所でマッチし、正確な % 位置でアニメーションが発火する。

---

### 7-4. `を確認する` だけでは step1 にも当たる

HTML に step1:`真数条件を確認する`、step2:`底を確認する` が共存するとき、
強調対象から LaTeX 除去後に残るキーワードが `を確認する` のみだと
DOM 順で先にある step1 が選ばれる。

**対処：**
- `底を確認する`（LaTeX `$a$` を除いた 4 文字以上の平文）を strong テキストとして HTML に使う
- 強調対象もその文字列をそのまま使う

---

### 7-5. 類題解答を初期非表示にするには `.step` クラスが必要

`screenshot_slides()` は類題スライドの解答エリアを
`document.querySelectorAll('.slide.active .step, .slide.active .answer-label, .slide.active .answer-box')`
で非表示にする。**CSS クラスが `step-box` だけでは非表示にならない。**

**対処：** 類題スライドの解答コンテナに `class="... step"` を追加する。
```html
<!-- 解答全体を包む div に class="step" を追加 -->
<div class="graph-wrap step" style="...">
```

---

### 7-6. 対応している方法（方法：フィールド）
...
### 10-1. GEMINI_TTS_STYLE_PREFIX の「やや遅めのテンポ」指示が余白を長くする

**症状：** 動画の前半（フック〜よくある間違いスライド）で、文と文の間の余白が長く聞こえる。後半（解説・一般化スライド）と比べてテンポが遅い印象になる。

**原因：** `GEMINI_TTS_STYLE_PREFIX` に「やや遅めのテンポで区切って発音する」という指示があると、TTS が文末（`。`）ごとに長めの間を挿入する。短い文が多い前半スライドで特に顕著になる。

**対処法：**
- `GEMINI_TTS_STYLE_PREFIX` から「やや遅めのテンポで区切って発音する」を**削除**する
- 代わりに「文と文の間の余白は短めにし、テンポよく畳みかけるリズムで読む」を追加する
- `GAP_SECONDS`（スライド切り替え後の無音）を `1.2` → **`0.8`** に変更する

**確認済み正しい設定（slide_to_video.py）：**
```python
...
GAP_SECONDS = 0.8  # 1.2 は長すぎる
```

チェックリスト追加：
- [ ] `GEMINI_TTS_STYLE_PREFIX` に「やや遅めのテンポで区切って」が残っていないか
- [ ] `GAP_SECONDS` が `0.8` になっているか（`1.2` は長すぎる）
