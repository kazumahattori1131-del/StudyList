...
すべての動画はサムネイル画像 + 以下の台本でイントロクリップを生成する。
台本は `tools/slide_to_video.py` の `INTRO_TEXT` で定義。**内容を変更しないこと。**
...
- サムネイル（`problems/youtube_redesign/thumbnails/{stem}.png`）が存在する場合のみ生成される
- サムネイルが存在しない場合はイントロがスキップされ警告が出る → 必ずサムネイルを先に生成すること
...
problems/youtube_redesign/voice_scripts/math2-NN_slug_voice.md
...
**参照ファイル**: `problems/youtube_redesign/voice_scripts/math2-10_log_inequality_voice.md`
...
タイムスタンプは `【キーワード】説明` 形式で視聴者向けに記述する。スライド名そのまま（「フック」「問題提示」など）は NG。

```
▼目次
0:00 【落とし穴】○○と書いた人、要注意！
1:03 【問題】問題文の要約
1:47 【よくある間違い】間違いの核心を一言で
2:25 【本質ポイント】正しい手順の概要
3:15 【解説】計算の流れを一言で
4:03 【一般化】パターンまとめの概要
5:03 【類題】類題の条件を一言で
6:22 【まとめ】今日の鉄則を一言で
...
```
|a+b|=5と書いた瞬間に0点。（具体的な数学的解決策）｜【数学X・単元名】
```

**構成順（固定）：**
1. **数学表記の誤答**（受験生が実際に書く式・数値を冒頭に。`3+2=5` のような算数表記は避け `|a+b|=5` のような数学的表記を使う）
2. **特定動作への結果提示**（「〜と書いた瞬間に0点。」「〜は0点です。」「〜と書いたら失点。」など、断定より動作→結果の形で誠実に注意喚起する）
3. **具体的な数学的解決策**（「2乗の鉄則」「内積展開の4ステップ」「不等号逆転の理由」など。「成績が伸びない」などの抽象表現は使わない）
4. **【数学X・単元名】**（検索性のため末尾に置く）

> スマホのサムネイル下テキストは冒頭15文字前後しか見えない。
> 冒頭15文字以内に「受験生が実際に書いてしまう数学表記の誤答」を入れること。
> 「爆死」「罠」などの煽りワードは京大生ブランドに合わないため使わない。
> 「0点です」と単独で断定するより「〜と書いた瞬間に0点。」と動作に紐づける方が誠実で刺さる。
...
| mathB-07 | |a+b|=5と書いた瞬間に0点。2乗の鉄則と内積展開でベクトルの大きさを正確に求める｜【数学B・ベクトル】 | 数学表記の誤答＋動作→結果＋具体的解決策 |

**タイトルのルール：**
- 冒頭は数学表記の誤答（`|a+b|=5` `x>2` `f'(x)=0だけ` など）。算数的な `3+2=5` は避ける
- 否定は「〜と書いた瞬間に0点。」「〜は0点です。」「〜と書いたら失点。」の形で動作に紐づける
- 中盤に「2乗の鉄則」「内積展開の手順」など**具体的な数学的解決策**を置く
- 抽象表現（「成績が伸びない原因」「危険」）は使わない
- 煽りワード（「爆死」「罠」「やばい」）は使わない
- `【数学X・単元名】` は末尾に置く（検索性のため）
- LaTeX・数式記号は使わない。数式は Unicode か言葉で代替
- タイトルは70文字以内

**フックのパターン例：**
```
|a+b|=5と書いた瞬間に0点。2乗の鉄則と内積展開でベクトルの大きさを正確に求める｜【数学B・ベクトル】
x>2と書いたら失点。底が1未満で不等号が逆転する理由と正しい解法｜【数学II・対数】
f'(x)=0だけでは0点です。端点を見落とさない最大最小の4ステップ｜【数学II・微分】
```
...
**成功パターンの構成：**
- **メインコピー**：フックワード（「0点」「使わない」「元に戻る」「逆転」など）を大きく（「爆死」「罠」は使わない）
...
| 要素 | 効果 | 具体例 |
|------|------|--------|
| 落とし穴ワード | 「自分も間違えたかも」と感じさせる | 「0点」「見落とし」「逆転」 |
...
トーン：**穏やかな先輩**。教えるのではなく「分かち合う」姿勢で書く。

- **冒頭**：「このミス、実はみんなが通る道です…」と寄り添い、サムネイルの衝撃をフォローする
- **中盤**：「京大生の僕が意識している思考プロセスをシェアします」というスタンスで書く
- **キーワード補強**：「定期テスト」「共通テスト」「模試」を文脈を壊さないよう自然に1〜2箇所入れる。メリット提示部分に「定期テストや共通テストで頻出の〜も確実に解ける」という表現が効果的
- **結び**：「苦手な人ほど見てほしい」「一緒に攻略しましょう」で前向きに締める
...
常に以下の構成で10個程度作成する（各タグ先頭に `#`、カンマ区切り）。

```
#高校数学, #大学受験, #[単元名], #[トピック], #数学解説, #受験数学, #計算ミス, #ケアレスミス, #京大生, #[具体的な試験名（共テ・全統等）]
```
...
トーン：**誠実・フラットな先輩**。視聴者のミスを笑わず、事実ベースの共感＋前向きな励まし。

- 「白状して」など強い言葉は避け、「この間違いをする人は多い」「自分も通った道」という共感を示す
- 「ここが分かれば一歩前進」というポジティブな励ましで締める
- 丁寧な言葉遣い（〜です、〜ます）を基本とし、威圧感を与えない
- 絵文字は文末に1〜2個のみ（👍 ✨ 🙏 程度）、使いすぎない
- **問いかけ形式**で思考を促す（「最初に何と答えましたか？」）
- 解答・ヒントは一切書かない
- 最後に高評価・チャンネル登録への誘導を入れる
- **LaTeX・数式記号は使わない**。数式は平文・Unicode・言葉で書く（例：`|a|`、`√21`）
...
- **ターゲット層は10代**。動画全体を通して**文と文の間の余白を短め**にする。
  `GEMINI_TTS_STYLE_PREFIX` に「文と文の間の余白は短めにし、テンポよく畳みかけるリズムで読む」を含めること。
  `GAP_SECONDS`（スライド切り替え後の無音）は **`0.8` 秒**を基準とする（1.2秒は長すぎる）。
  `GEMINI_TTS_STYLE_PREFIX` から「やや遅めのテンポで区切って発音する」は**削除する**（余白が長くなる原因）。
- スライド①（フック）の音声は特に**テンポを速く**する。
  冒頭の無音・間は最小限にし、voice.md のスライド①は文を短く区切り、テンポよく畳みかけるリズムで書く。
...
- **サムネイル**：単元名だけでなく「〇〇すると0点」「逆転」などの落とし穴ワードと正解の対比を使う（「爆死」「罠」は使わない）
...
- サムネイルで使う**強いワード**（「逆転」「0点」「逆向き」など）を、
  Leda の**最初の一言に必ず採用**する。（「爆死」「罠」は使わない）
...
- 検索クエリに「逆」「なぜ」「間違い」「0点」が付きやすい単元ほど CTR が高い
...
2. **インパクトあるコピーライティング**: `top_copy` は10文字以内の強いワード（「不等号が逆転！」「書いた瞬間に0点！」など）。モバイル3秒で読める量。「爆死」「罠」は使わない
