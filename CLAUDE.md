...
```
具体的な間違い（数値・式）は間違いです。正しくは○○できる｜【数学X・単元名】
```

**構成順（固定）：**
1. **具体的な間違い**（受験生が実際にやりがちな数式・数値を冒頭に）
2. **知的な否定**（「間違いです」「0点です」「成績が伸びない原因」など論理的にNOを突きつける）
3. **動画のメリット**（正しく解けるようになること・時間が節約できることなど）
4. **【数学X・単元名】**（検索性のため末尾に置く）

> スマホのサムネイル下テキストは冒頭15文字前後しか見えない。
> 冒頭15文字以内に「受験生が実際に書いてしまう具体的な式・数値」を入れること。
> 「爆死」「罠」などの煽りワードは京大生ブランドに合わないため使わない。
> 知的・論理的なNO（「間違いです」「0点です」）で差別化する。
...
| mathB-07 | |a+b|=5は間違いです。内積展開で√21を正確に求める方法｜【数学B・ベクトル】 | 具体的な誤答＋知的な否定 |

**タイトルのルール：**
- 冒頭に「受験生が実際に書く誤答の式・数値」を置く（例：`3+2=5は` `x>2は` `|a+b|=5は`）
- 否定表現は知的・論理的に（「間違いです」「0点です」「成績が伸びない原因」）
- 煽りワード（「爆死」「罠」「やばい」）は使わない
- メリットは具体的な動詞で（「正確に求める」「一発で解ける」「瞬時に判断できる」）
...
**フックのパターン例：**
```
3+2=5は間違いです。ベクトルの大きさを正しく求める4ステップ｜【数学B・ベクトル】
x>2は0点です。対数不等式で不等号が逆転する理由と正しい解法｜【数学II・対数】
f'(x)=0だけでは不十分です。端点を見落とさない最大最小の手順｜【数学II・微分】
```
