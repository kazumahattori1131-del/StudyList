...
GEMINI_TTS_STYLE_PREFIX = (
    ...
    "若くて落ち着いた声で、テンポよくリズミカルに読む。"
    "文と文の間の余白は短めにし、テンポよく畳みかけるリズムで読む。"
    "カタカナ語（ベータ・アルファ・イコール・カッコ等）は日本語の発音で読む。"
    "数式の各項をひとつひとつ丁寧に区切って発音する。"
    "ポイント強調時は声にわずかな力を込める。\n"
)
...
OUTRO_TEMPLATE = """今回は「{title}」について解説しました。この解法を知っておくだけで、入試本番で同じタイプの問題が出たとき、迷わず手が動きます。典型パターンをひとつひとつ押さえていけば、確実に点数が安定します。役に立ったと感じた方は、チャンネル登録と高評価で応援してください！次回も、入試で差がつく重要ポイントを解説します。疑問点や解説してほしい問題があれば、コメントで教えてもらえると助かります。それでは！"""
